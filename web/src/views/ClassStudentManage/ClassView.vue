<template>
  ...
  <!-- 班级信息表格区域 -->
  <div class="tableInfo">
    <el-table :data="tableData" border :fit="true" :header-cell-style="{ background: '#f2f2f2', fontWeight: 'bold' }">
      <!-- 序号列，动态计算序号 -->
...
const rules = reactive({
  name: [
    ...
    { min: 4, max: 30, message: '班级名称长度需在 4 到 30 个字符之间', trigger: 'blur' },
    { pattern: /^[a-zA-Z0-9\u4e00-\u9fa5]+$/, message: '班级名称非法', trigger: 'blur' }
  ],
  classroom: [
    { min: 1, max: 20, message: '班级教室长度需在 1 到 20 个字符之间', trigger: 'blur' },
    { pattern: /^[a-zA-Z0-9\u4e00-\u9fa5]+$/, message: '班级教室非法', trigger: 'blur' }
  ],
  startTime: [
...
function validateTime(rule, value, callback) {
  if (!form.value.startTime || !form.value.endTime) {
    ...
    callback(new Error('开课时间不能晚于结课时间'));
  } else {
    callback();
  }
}
...
async function getClassList() {
  ...
  try {
    ...
    const tempData = res.data.data.rows;
    tempData.forEach(x => {
